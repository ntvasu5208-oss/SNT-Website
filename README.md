...
3. Click **Extensions → Apps Script**. Delete anything in the editor and paste this in:
   ...
   *(The site sends this as a background image request rather than a form submission — it's the same well-established technique used by email-open trackers, and it's the most reliable way to fire a request that survives the page switching away right after, like when WhatsApp opens on mobile. `doGet` handles that; `doPost` is kept too in case anything else calls this later.)*
